...
// ── Deflate nominal future dollars to today's dollars ─────────────────────────
// Converts a balance curve from nominal future values to real (today's) dollars.
// This matches ASIC MoneySmart's approach and the ASIC regulatory requirement
// that superannuation forecasts be presented in today's dollars.
// Formula: real_value = nominal_value / (1 + inflation)^years_from_now
...
export default function ForecastResultPage() {
  ...
  useEffect(() => {
    ...
    // Deflate curves to today's dollars for the chart
    ...
    // Spending line: safe spending uses balanced amount; traditional uses user's input
    const isSafeChart = (inputs.mode || 'traditional') === 'safe_spending';
    const spendingAmt = isSafeChart ? (outputs.safe_spending_balanced || 0) : (inputs.annual_spending || 0);
      ...
      chartInstance.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            ...
            { label: isSafeChart ? 'Balanced safe spending' : 'Spending target (today\'s $)', data: spendingLine, borderColor: 'rgba(255,255,255,0.2)', borderWidth: 1, borderDash: [3, 6], pointRadius: 0, fill: false, tension: 0 },
          ],
        },
        options: {
          responsive: true,
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                afterBody: (items) => {
                  const age = parseInt(items[0].label.replace('Age ', ''));
                  if (age === pensionAge) return ['', '  ⚓ Age Pension begins at this age'];
                  return [];
                },
              },
            },
      ...
      <>
        ...
        <div style={{ minHeight: '100vh', background: '#0D1F35', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center', color: '#e08878', fontFamily: 'DM Sans, sans-serif' }}>
            ...
            <button onClick={() => { window.location.href = '/dashboard'; }} style={{ marginTop: 20, padding: '10px 24px', background: '#c9a84c', border: 'none', borderRadius: 4, color: '#0D1F35', fontFamily: 'DM Sans, sans-serif', cursor: 'pointer', fontWeight: 600 }}>
              Back to dashboard
            </button>
          </div>
        </div>
  ...
  const isSafeSpending = (inputs.mode || 'traditional') === 'safe_spending';
  ...
  const retirementBalanceReal    = deflateScalar(retirementBalanceNominal, yearsToRetirement, inflation);

  // Pension is already in today's rates (calculated on current Centrelink rates)
  ...
  // Safe spending outputs (present when mode === 'safe_spending')
  ...
  const pensionCardSub = () => {
    if (pensionAnnual > 0) {
      return <>{fmtFull(pensionFortnightly)} per fortnight from age 67<br />based on assets &amp; income tests</>;
    }
    if (pensionEligibleFromAge) {
      return <>Projected balance exceeds threshold at 67<br />Eligible from approximately <strong style={{ color: '#7ec896' }}>age {pensionEligibleFromAge}</strong> as your balance reduces</>;
    }
    return <>Not eligible from age 67<br />Projected balance exceeds the assets or income test threshold</>;
  };

  // ── Shared nav ────────────────────────────────────────────────────────────
  const nav = (
    <nav className="hf-nav">
      <a href="/dashboard" className="hf-logo">
        ...
        <div className="hf-root">
          ...
          <div className="hf-page">
            ...
            <div className="hf-stat-grid">
              ...
              {yearsToRetirement > 0 ? (
                <div className="hf-stat-card hf-stat-blue">
                  ...
                  <div className="hf-stat-value blue">{retirementBalanceReal ? fmt(retirementBalanceReal) : '—'}</div>
            ...
            <div className="hf-chart-section">
              ...
              <div className="hf-chart-note">Balanced spending of {safeBalanced ? fmtFull(safeBalanced) : '—'}/yr · all values in today's dollars · 1,000 simulated scenarios · {inputs.has_partner ? 'Couple homeowner' : 'Single homeowner'}</div>
            </div>
        ...
        <div className="hf-page">
          ...
          <div className="hf-stat-grid">
            ...
            <div className="hf-stat-card hf-stat-green">
              ...
              <div className="hf-stat-sub">{pensionCardSub()}</div>
            </div>
            ...
            <div className="hf-stat-card hf-stat-blue">
              ...
              <div className="hf-stat-sub">
                Median scenario<br />
                Best case {fmtAge(fundsLastP90)} · Worst case {fmtAge(fundsLastP10)}
              </div>
            </div>
          </div>
          ...
          <div className="hf-chart-section">
            ...
            <div className="hf-chart-wrap"><canvas ref={chartRef} /></div>
            <div className="hf-chart-note">
              All values in today's dollars (2.5% inflation adjustment) · 1,000 simulated scenarios · {inputs.has_partner ? 'Couple homeowner' : 'Single homeowner'} · Returns net of {0.67}% p.a. fees
            </div>
          </div>
          ...
          <div className="hf-summary-section">
            <div className="hf-summary-label">What this forecast shows</div>
            <div className="hf-summary-text">
              Based on a current super balance of <strong>{fmtFull(superBalance)}</strong> at age <strong>{currentAge}</strong>, with a target retirement age of <strong>{retirementAge}</strong>, Harbour projects a median super balance of approximately <strong>{retirementBalanceReal ? fmt(retirementBalanceReal) : '—'} in today's dollars</strong> at retirement. From age <strong>67</strong>, {pensionAnnual > 0
...
const safeSpendingStyles = `
  ...
  .ss-pension-title { font-size: 13px; font-weight: 600; color: #f5f0e8; margin-bottom: 4px; }
...
const baseStyles = `
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .hf-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; flex-wrap: wrap; gap: 16px; }
